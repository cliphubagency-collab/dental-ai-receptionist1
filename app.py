...
def check_slots(date):
    try:
        events = calendar.events().list(calendarId=CALENDAR_ID, timeMin=f"{date}T00:00:00Z", timeMax=f"{date}T23:59:59Z", singleEvents=True).execute()
...
def tools():
    ...
    if name == "check_slots":
        slots = check_slots(params["date"])
        return jsonify({"success": True, "result": f"Available: {', '.join(slots)}"})
    if name == "book_appointment":
        msg = book_appointment(params["name"], params["phone"], params["date"], params["time"], params["service"])
